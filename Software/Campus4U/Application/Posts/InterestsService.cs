...
public sealed class InterestsService(IInterestsRepository interestsRepository, IPostsRepository postsRepository)
{
    ...
    public async Task<InterestToggleResult> ToggleAsync(int postId, int userId, CancellationToken ct = default)
    {
        ...
        if (eventDate is null) return new InterestToggleResult(false, false, "Objava ne postji");
        if (eventDate.Value == default) return new InterestToggleResult(false, false, "Objava se ne odnosi na događaj");
        
        var isInterested = await interestsRepository.IsInterestedAsync(postId, userId, ct);
